...
int main() {
    ...
    rat::mdl::ShSerializerPr serializer = rat::mdl::Serializer::create(json_file_path);

    if (!serializer->has_valid_json_root()) {
        std::cerr << "Invalid JSON root in file: " << json_file_path << std::endl;
        return -1;
    }
    rat::mdl::ShModelGroupPr model = std::dynamic_pointer_cast<rat::mdl::ModelGroup>(serializer->construct_tree_core());
    ...
    const bool run_json_export = false;
