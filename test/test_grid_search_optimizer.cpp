...
TEST_F(GridSearchOptimizerTest, hasDriveValueChanged)
{
    ...
    // Initial parameters
    CCTools::HarmonicDriveParameterMap drive_values_before = model_handler.getHarmonicDriveValues();

    // Call method and expect no change initially
    ASSERT_FALSE(opt_alllinear->hasDriveValueChanged(drive_values_before));

    // Change parameters
    CCTools::HarmonicDriveParameterMap new_params;
    new_params["B1"] = CCTools::HarmonicDriveParameters(drive_values_before["B1"].getOffset() + 0.001, drive_values_before["B1"].getSlope() + 0.0001);
...
TEST_F(GridSearchOptimizerTest, checkLengthSanity)
{
    ...
    // Initial check
    CCTools::HarmonicDriveParameterMap fallback_drives = model_handler.getHarmonicDriveValues();
    ASSERT_NO_THROW(opt_alllinear->checkLengthSanity(fallback_drives));

    // Change B1 params to unreasonable values
    CCTools::HarmonicDriveParameterMap new_params;
    new_params["B1"] = CCTools::HarmonicDriveParameters(-0.0134697, 0.0155);
    model_handler.apply_params(new_params);
...
TEST_F(GridSearchOptimizerTest, setParamRanges)
{
    ...
    CCTools::HarmonicDriveParameterMap drive_values = model_handler.getHarmonicDriveValues();
